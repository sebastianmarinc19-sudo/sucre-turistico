...
// La usan las pruebas, y sirve tambien para levantar el gateway sin base de
// datos mientras se trabaja en el front-end.
function crearRepoEnMemoria(iniciales = []) {
  ...
  return {
    ...
    async crear({ nombre, email, passwordHash }) {
      const usuario = { id: siguienteId++, nombre, email, password_hash: passwordHash };
